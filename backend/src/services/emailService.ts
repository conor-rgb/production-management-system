...
export async function getThreads(options: ThreadListOptions) {
  ...
  if (options.search) {
    ...
    andFilters.push({
      OR: [
        ...
        { messages: { some: { isDuplicateSuppressed: false, isDraftArtifact: false, bodyText: { contains: search, mode: "insensitive" } } } },
        { messages: { some: { isDuplicateSuppressed: false, isDraftArtifact: false, fromName: { contains: search, mode: "insensitive" } } } },
        { messages: { some: { isDuplicateSuppressed: false, isDraftArtifact: false, fromAddress: { contains: search, mode: "insensitive" } } } },
      ],
    });
  }
  ...
  switch (folder) {
    case "sent":
      andFilters.push({ OR: [{ inSent: true }, { messages: { some: { isFromMe: true, isDuplicateSuppressed: false, isDraftArtifact: false } } }] });
      break;
    case "flagged":
    case "starred":
    ...
    default:
      andFilters.push({
        OR: [
          { inInbox: true },
          { isArchived: false, messages: { some: { isFromMe: false, isDuplicateSuppressed: false, isDraftArtifact: false } } },
        ],
        isTrashed: false,
      });
      break;
  ...
  if (folder === "sent") {
    const sentWhere: Prisma.EmailMessageWhereInput = { isFromMe: true, isDuplicateSuppressed: false, isDraftArtifact: false, thread: { is: where } };
    ...
    const [foundThreads, latestSentMessages] = await Promise.all([
      ...
      prisma.emailMessage.findMany({
        where: { threadId: { in: orderedIds }, isFromMe: true, isDuplicateSuppressed: false, isDraftArtifact: false },
        orderBy: { sentAt: "desc" },
        select: {
          threadId: true,
          fromAddress: true,
  ...
  const attachmentRows = threadIds.length
    ? await prisma.emailMessage.findMany({
      where: { threadId: { in: threadIds }, hasAttachments: true, isDuplicateSuppressed: false, isDraftArtifact: false },
...
export async function getThread(threadId: string, options?: { before?: Date; limit?: number; messageId?: string }) {
  ...
  const [totalMessageCount, olderTotalCount, recentMessagesDesc, allAttachmentsMessages] = await Promise.all([
    prisma.emailMessage.count({ where: { threadId, isDuplicateSuppressed: false, isDraftArtifact: false } }),
    ...
    prisma.emailMessage.findMany({
      where: { threadId, hasAttachments: true, isDuplicateSuppressed: false, isDraftArtifact: false },
      orderBy: { sentAt: "asc" },
