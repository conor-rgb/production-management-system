...
Captured before implementation on 16 September 2026; retained as a dated baseline. Later source fixes and deployment are recorded in [the release record](../releases/2026-09-16-production-hub.md). Statements below describe the audited baseline, not necessarily the current code. Companion to [the consolidation plan](PLAN.md). Scope: repository inspection, read-only database measurements, selected Google Sheets ranges, two standalone Apps Script files and extracted text from sample published PDFs. No live sheet or database records were changed. This is not a complete accounting audit or a review of every bound script.
