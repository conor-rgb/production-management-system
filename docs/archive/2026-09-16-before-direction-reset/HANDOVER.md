# HANDOVER - 2026-06-08 - VPS Storage Cleanup Note

## Built This Session
- Cleaned safe VPS caches and temp files:
  - npm cache and `_npx`,
  - Playwright browser cache,
  - VS Code cached extension installers,
  - inactive old VS Code server versions,
  - old Codex session logs older than 14 days,
  - Codex temp files,
  - apt package lists/cache,
  - temp `.ts-node*` folders.
- Added a persistent note to `CLAUDE.md` under “VPS storage maintenance”.

## Result
- Root disk usage went from `17G used / 31G free` to `11G used / 37G free`.
- Current root usage after cleanup: about `23%`.

## Important Note
- Because apt package lists were cleaned, run `apt-get update` before the next `apt-get install`.
- Do not delete app uploads in `backend/storage`, PostgreSQL data in `/var/lib/postgresql`, `/var/www/agent`, or the active VS Code server version.
