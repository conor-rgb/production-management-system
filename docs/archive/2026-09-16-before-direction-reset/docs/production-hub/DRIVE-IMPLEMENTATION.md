...
Implemented 16 September 2026. This changes the application source; it has not been deployed or connected to the user's live Google account. Existing Drive folders were inspected read-only. Tests use an isolated database schema and mocked Google requests.
...
3. Enable Google Drive API for the existing Google Cloud OAuth client. Register this exact additional authorised redirect URI:
   `https://agent.unlimited.bond/api/drive/oauth/callback`
...
The Drive-specific callback was missing in the inspected server environment. Other Google client and encryption settings were present; their secret values were not printed. Google consent and live publishing have not been exercised.

## Backend behaviour

`DriveConnection` is workspace-scoped, matching the existing single-team authentication model. Refresh tokens are encrypted using the application's encryption service. OAuth uses a ten-minute session-bound random state, validates the granted scope, verifies root access and consumes state before exchanging the code. Session cookies use SameSite=Lax to support the top-level Google callback; HttpOnly and production Secure remain enabled.
