...
Prepared 16 September 2026. Status: proposal grounded in the current repository, read-only database measurements and selected live Drive references. This document does not claim the redesign has been implemented or deployed.

Implementation update: the workspace index, finance navigation and Drive file/export foundation are now implemented in source. See [implementation and activation notes](DRIVE-IMPLEMENTATION.md) for verified scope and remaining work. The full financial-ledger redesign below remains a proposal.
...
The first UI cleanup in this conversation improved entry points, but Home, Actions and command search still call the existing oversized project endpoint. Those new screens need the performance work below too.
...
Keep PostgreSQL as operational truth and Drive as file truth. For the first performance/finance slice, retain the current React/Express/Prisma stack. The original proposal requests Next.js/Supabase; the later Drive master describes a VPS/Postgres direction. Record the deployment/auth decision before a platform migration. Changing frameworks alone will not fix the measured payload or financial ownership problems.
...
Next implementation target: **replace the oversized project reads, then build Costs as the single daily workspace for estimate, commitments, invoices and reconciliation.**
