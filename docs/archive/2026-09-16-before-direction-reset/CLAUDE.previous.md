...
## VPS storage maintenance
- 2026-06-08 cleanup reduced root disk from 17G used to 11G used.
- Safe cleanup targets used: npm cache, npm `_npx`, Playwright browser cache, VS Code cached extension VSIXs, inactive old VS Code server versions, old Codex sessions older than 14 days, Codex temp files, apt package lists/cache, and temp `.ts-node*` folders.
- Do not delete: `/srv/production-management-system/backend/storage`, PostgreSQL data in `/var/lib/postgresql`, `/var/www/agent`, or the active VS Code server version.
